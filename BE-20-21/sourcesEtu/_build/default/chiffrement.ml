...
struct
  ...
  let dechiffrer list arbre = 
    ...
    let rec aux code arbre arbre_init = 
      match code, arbre with
      | _, Lettre e -> ([e], code, arbre_init)
      | symbole::reste_code,  arbre ->
          (match get_branche symbole arbre with
          | Some  branche -> aux reste_code branche arbre_init
          | None -> raise CodeNonValide)
      | _, _ -> raise CodeNonValide

    in

    let rec aux1 code arbre decode = 
      match code with 
      | [] -> decode
      | _ ->  
          let (e, code_restant, arbre_init) = aux code arbre arbre in 
          aux1 code_restant arbre_init (decode@e)
    in 
